...
# indent = space - you can set this in your editor config
# tab-size = 2
...
if stdargs.version:
  print(f'warden version: {VERSION}\n'
...
DEFAULT_CONF: Template = Template(f'#? Config file for warden v. {VERSION}' + '''

#* Color theme, looks for a .theme file in "/usr/[local/]share/warden/themes" and "~/.config/warden/themes", "Default" for builtin default theme.
...
#* Set to 2 or higher to greatly decrease warden cpu usage. (Only integers)
...
CONFIG_DIR: str = f'{os.path.expanduser("~")}/.config/warden'
...
class Config:
  ...
  def __init__(self, path: str):
    ...
    if not "version" in conf.keys():
      ...
      self.recreate = True
      self.info.append(f'Config file version and warden version mismatch, will be recreated on exit!')
...
try:
  ...
  errlog.info(f'New instance of warden version {VERSION} started with pid {os.getpid()}')
...
class Banner:
  '''Holds the warden banner, .draw(line, [col=0], [center=False], [now=False])'''
  out: List[str] = []
  c_color: str = ""
  length: int = 0
  if not out:
    ...
    categories: Dict[str, Dict[str, List[str]]] = {
      "system" : {
        "color_theme" : [
          ...
          '"/usr/[local/]share/warden/themes" and',
          '"~/.config/warden/themes".',
          '',
      ...
      "proc" : {
        "proc_update_mult" : [
          ...
          '',
          'Set to 2 or higher to greatly decrease warden',
